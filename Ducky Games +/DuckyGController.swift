...
class DuckyGController: NSObject, WKNavigationDelegate {
    let owner: DuckyGameEngineLoader
    var redirectFlag = false
  
    init(owner: DuckyGameEngineLoader) {
        self.owner = owner
        debugPrint("Control init")
    }
    
    private func updateState(_ state: DuckyLState) {
        DispatchQueue.main.async { [weak self] in
            self?.owner.vm.lstate = state
        }
    }
